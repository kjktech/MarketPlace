...
<div class="card mb-4">
                <h6 class="card-header bg-white">Images (680 X 460)</h6>
                <div class="card-body">
...
<script>
        ...
        $('#fine-uploader-gallery').fineUploader({
            ...
            request: {
                endpoint: '{{route('post.upload', $listing)}}',
                customHeaders: {
            ...
            session: {
                endpoint: '{{route('post.session', $listing)}}',
                customHeaders: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            },
            ...
            validation: {
                ...
                image: {minWidth: 638, minHeight: 450},
                itemLimit: 4,
            },
            deleteFile: {
                enabled: true,
                forceConfirm: true,
                endpoint: '{{route('post.delete-image', $listing)}}',
                customHeaders: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            },
